...
from daterange_filter.filter import (
    DateRangeFilterBaseForm,
    DateRangeForm,
    DateTimeRangeForm,
...
class DateRangeFormTest(BaseTest):
    ...
    def test_field_attributes(self):
        ...
        self.assertIsInstance(
            form.fields['drf__ham__gte'].widget,
            AdminDateWidget
        )
        ...
        self.assertIsInstance(
            form.fields['drf__ham__lte'].widget,
            AdminDateWidget,
        )
        self.assertDictContainsSubset(
...
class DateRangeFilterTest(BaseTest):
    def setUp(self):
        self.request = Mock()
        self.filter_ = DateRangeFilter(
            'spam', self.request, [], Mock(), Mock(), 'egg'
        )
    ...
    def test_get_form(self, DateRangeForm):
        ...
        self.assertEqual(
            [call(self.request, data=ANY, field_name='egg')],
    ...
    def test_queryset_ignore_null_fields(self):
        ...
        filter_ = DateRangeFilter(
            'spam', self.request, params, Mock(), Mock(), 'ham'
        )
    ...
    def test_queryset_filters_by_date_from(self):
        ...
        self.assertEqual(return_value, queryset.filter.return_value)
        self.assertItemsEqual(
            [call(ham__gte=date(2014, 1, 3))], queryset.filter.call_args_list)
...
class DateTimeRangeFilterTest(BaseTest):
    ...
    def test_get_form(self, DateTimeRangeForm):
        ...
        self.assertEqual(
            [
                call(
                    self.request,
                    data={'drf__egg__lte_0': None, 'drf__egg__lte_1': None},
                    field_name='egg'
                )
            ],
    ...
    def test_queryset_filters_by_date_from(self):
        ...
        self.assertEqual(return_value, queryset.filter.return_value)
        self.assertItemsEqual(
            [call(ham__gte=timezoned_expected_result)],
            queryset.filter.call_args_list,
        )

    def test_queryset_filters_by_date_to(self):
        queryset = Mock()
        data_end = timezone.make_aware(
        ...
        self.assertEqual(return_value, queryset.filter.return_value)
        self.assertEqual(
            data_end, queryset.filter.call_args_list[0][1]['ham__lte'])
